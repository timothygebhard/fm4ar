...
class ContinuousFlowModel(nn.Module):
    ...
    def _update_cached_context(self, *context):
        ...
        if all([(x == x[:1]).all() for x in context]):
            ...
            self._cached_context_embedding = context_embedding.repeat(
                batch_size, *[1 for _ in range(len(context_embedding.shape) - 1)]
            )
        else:
    ...
    def forward(self, t, theta, *context):
        ...
        if not self.use_cache:
            ...
            context_embedding = self._cached_context_embedding
